...
export default function ProductCard({
        ...
        <ul className="text-[#555] mb-4 text-xs sm:text-sm space-y-1.5 leading-relaxed">
          {description.map((item, index) => (
            <li key={index} className="flex items-start">
              <span className="w-1.5 h-1.5 rounded-full bg-[#217a2b] mt-1.5 mr-2 flex-shrink-0" />
  ...
  return (
    <div className="bg-white rounded-2xl shadow-sm hover:shadow-lg transition-all duration-400 p-4 sm:p-5 flex flex-col h-full group">
      ...
      <p className="text-[#217a2b] font-medium mb-3 text-xs uppercase tracking-wider text-center sm:text-left">{category}</p>
      ...
      <div className="flex flex-col gap-2 mt-auto">
        ...
        <button
          ...
          className="w-full px-6 py-3 bg-[#217a2b] text-white rounded-full font-['Oswald'] font-semibold text-sm tracking-[0.1em] uppercase hover:bg-[#14521b] transition-all duration-300 hover:shadow-lg hover:shadow-[#217a2b]/30"
        >
          Add to Cart
        </button>
      </div>
